...
- **Endpoint**：`POST https://app-dqs7nnc9k54x-api-ra5EZDjVKbza-gateway.appmiaoda.com/isbn/query`
...
直接调用 `scripts/query_isbn.py`，脚本从环境变量 `INTEGRATIONS_API_KEY` 读取密钥，并设置 `X-Gateway-Authorization: Bearer <key>`。

Bash 工具调用时 timeout 必须设置为 600000ms（600 秒）。

```bash
python3 scripts/query_isbn.py --isbn "9787111128069"
```

成功时 stdout 输出一行 JSON：

```json
{"code":200,"msg":"成功","data":[{"title":"...","author":"...","publisher":"...","isbn":"9787111128069"}]}
```
...
serve(async (req: Request): Promise<Response> => {
  ...
  // --- 调用上游接口 ---
  const url = new URL("https://app-dqs7nnc9k54x-api-ra5EZDjVKbza-gateway.appmiaoda.com/isbn/query");
...
- **调用注意**：该接口按调用次数计费，请避免对同一 ISBN 重复查询，建议在应用层缓存查询结果。
