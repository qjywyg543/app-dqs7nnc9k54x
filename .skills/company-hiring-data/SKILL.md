...
| 接口 | Endpoint | 返回内容 |
|------|----------|---------|
| 招聘概况查询 | `POST https://app-dqs7nnc9k54x-api-Aa2PZ2MejdoL-gateway.appmiaoda.com/enterprise/hire-overview` | 职位关键词、职位类型、城市分布、招聘来源、职位级别、平均薪资、职位数量等概况 |
| 招聘数据统计 | `POST https://app-dqs7nnc9k54x-api-zYkZzErqJg4L-gateway.appmiaoda.com/enterprise/hire-statistics` | 平均薪资、学历分布、主要省份、招聘职位、薪资区间、工作年限分布、招聘数量时间序列等详细统计 |
...
两个接口均只需传入一个 `keyword` 参数，可独立调用。详细参数表、响应字段说明见：

- 招聘概况查询：`references/hire-overview-api.md`
- 招聘数据统计：`references/hire-statistics-api.md`

直接调用 `scripts/*.py`，脚本从环境变量 `INTEGRATIONS_API_KEY` 读取密钥，并设置 `X-Gateway-Authorization: Bearer <key>`。

Bash 工具调用时 timeout 必须设置为 600000ms（600 秒）。

```bash
python3 scripts/query_hire_overview.py --keyword "北京百度网讯科技有限公司"
python3 scripts/query_hire_statistics.py --keyword "北京百度网讯科技有限公司"
```
