...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-5YrZz81oerkY-gateway.appmiaoda.com/rest/2.0/face/v3/match` |
| Auth 模式 | `platform_managed`（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer ${apiKey}` |
| Content-Type | `application/json` |
| third_part_domain | `app-dqs7nnc9k54x-api-5YrZz81oerkY-gateway.appmiaoda.com` |
| 计费 | 启用，按调用次数计费 |
...
```bash
# 两张本地图片
python3 <skill-path>/scripts/call_face_comparison.py --image1 /path/to/a.jpg --image2 /path/to/b.jpg [--face-type LIVE] [--quality-control LOW] [--liveness-control HIGH]

# 使用 FACE_TOKEN
python3 <skill-path>/scripts/call_face_comparison.py --image1-token <face_token_1> --image2-token <face_token_2>
```

脚本成功时 stdout 输出一行 JSON（即上游接口原始响应，含 `result.score`、`result.face_list` 等字段）。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-5YrZz81oerkY-gateway.appmiaoda.com/rest/2.0/face/v3/match",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
3. **计费**: 每次调用均计费，请避免不必要的重复调用（如用户未确认时勿频繁发送）。
