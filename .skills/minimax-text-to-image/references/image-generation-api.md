...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-DLEO7vB8pQba-gateway.appmiaoda.com/v1/image_generation` |
| 认证模式 | `platform_managed` |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/json` |
| Third-party Domain | `app-dqs7nnc9k54x-api-DLEO7vB8pQba-gateway.appmiaoda.com` |
...
生成期调用已收敛到脚本，避免模型现场生成请求代码和下载命令。

```bash
python3 <skill-path>/scripts/generate_image.py \
  --prompt "A scenic mountain landscape at sunset, ultra-realistic, 8K." \
...
2. 请求 `response_format=url`
3. 从响应 `data.image_urls` 获取临时 URL
4. 立即下载到 `--output` 或 `--output-dir`
5. stdout 输出一行 JSON
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-DLEO7vB8pQba-gateway.appmiaoda.com/v1/image_generation",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
