...
- **Endpoint**：`GET https://app-dqs7nnc9k54x-api-oLpZ74noWOMa-gateway.appmiaoda.com/idcard`
...
async function verifyIdCardTwoFactor(
  ...
  const response = await fetch(
    `https://app-dqs7nnc9k54x-api-oLpZ74noWOMa-gateway.appmiaoda.com/idcard?${params.toString()}`,
    {
      method: "GET",
      headers: {
...
- **计费**：每次成功调用（`ret_code=0`）计费 1 次。`ret_code≠0` 的失败调用不计费，请在业务层做好参数校验，避免无效调用。
