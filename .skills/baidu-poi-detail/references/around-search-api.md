...
| Endpoint | `GET https://app-dqs7nnc9k54x-api-DLEO7eMnzMwa-gateway.appmiaoda.com/place/v3/around` |
| Auth | `X-Gateway-Authorization: Bearer <INTEGRATIONS_API_KEY>`（platform_managed） |
| Content-Type | 无请求体（GET 参数通过 URL 查询串传递） |
| Third-party domain | `app-dqs7nnc9k54x-api-DLEO7eMnzMwa-gateway.appmiaoda.com` |
| 计费 | 按次计费 |
...
脚本内部负责读取 `INTEGRATIONS_API_KEY`、发送请求并输出周边检索结果。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-dqs7nnc9k54x-api-DLEO7eMnzMwa-gateway.appmiaoda.com/place/v3/around?${params.toString()}`,
    {
      method: "GET",
      headers: {
