...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-rY7JZ6jqr6dL-gateway.appmiaoda.com/v2/ai_search/chat/completions` |
| Content-Type | `application/json` |
| 认证模式 | `platform_managed`（`X-Gateway-Authorization: Bearer <INTEGRATIONS_API_KEY>`） |
| third_part_domain | `app-dqs7nnc9k54x-api-rY7JZ6jqr6dL-gateway.appmiaoda.com` |
| 计费 | 计费信息以平台实际配置为准 |
...
脚本内部负责读取 `INTEGRATIONS_API_KEY`、构造 `messages`/`resource_type_filter`/`search_recency_filter`、发送请求并输出 `references` 结果。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-rY7JZ6jqr6dL-gateway.appmiaoda.com/v2/ai_search/chat/completions",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
