...
| 属性 | 值 |
|------|----|
| 主调用接口 | `POST https://app-dqs7nnc9k54x-api-rLobPAn0n7m9-gateway.appmiaoda.com/v1/chat/completions` |
| Endpoint 形式 | `https://{APP_ID}@app-dqs7nnc9k54x-api-rLobPAn0n7m9-gateway.appmiaoda.com/v1/chat/completions` |
| 官方来源接口 | `POST https://app-dqs7nnc9k54x-api-rLobPAn0n7m9-gateway.appmiaoda.com/v1/chat/completions`（仅用于接口来源说明，Skill 调用使用上方 `APP_ID@` 网关形式） |
...
生成期直接调用脚本 `scripts/call_minimax_m3_chat.py`，脚本内完成密钥读取、请求、解析，Agent 只需构造命令、读取输出 JSON。**Bash 工具调用超时必须设置为 `600000ms`（600 秒）。**

```bash
# 单轮对话
python3 scripts/call_minimax_m3_chat.py --prompt "你好，请用一句话介绍你自己"

# 自定义模型/参数与思考模式
python3 scripts/call_minimax_m3_chat.py --prompt "..." \
  --temperature 1 --top-p 0.95 --max-completion-tokens 2048 --thinking adaptive

# 完整 messages（多轮/多模态）输入，覆盖 --prompt
python3 scripts/call_minimax_m3_chat.py --messages '[{"role":"user","content":"你好"}]'
```

成功时 stdout 输出一行 JSON：`{"status":"succeed","result":{...上游原始响应...}}`。

多模态（图片/视频理解）请用 `--messages` 传入符合上游格式的 `content` 数组；参数细节详见 [references/chatcompletion-api.md](references/chatcompletion-api.md)。生成期调用实现以 `scripts/call_minimax_m3_chat.py` 为准。
