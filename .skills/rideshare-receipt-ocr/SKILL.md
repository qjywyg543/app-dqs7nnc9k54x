...
- **Endpoint**: `POST https://app-dqs7nnc9k54x-api-zYkZz8qoKp1L-gateway.appmiaoda.com/rest/2.0/ocr/v1/online_taxi_itinerary`
...
生成期调用请使用内置脚本，脚本会从环境变量读取 `INTEGRATIONS_API_KEY`，并按 `image > url > pdf_file > ofd_file` 优先级处理输入。
...
python3 <skill-path>/scripts/call_online_taxi_itinerary_ocr.py --url "https://example.com/rideshare-receipt.jpg"
python3 <skill-path>/scripts/call_online_taxi_itinerary_ocr.py --pdf-file /path/to/rideshare-receipt.pdf --pdf-file-num 1
python3 <skill-path>/scripts/call_online_taxi_itinerary_ocr.py --ofd-file /path/to/rideshare-receipt.ofd --ofd-file-num 1
```
