...
| Third-part Domain | `app-dqs7nnc9k54x-api-2Y00VV8Rkb2Y-gateway.appmiaoda.com` |
| 计费 | 按次计费 |
...
| 端点 | `POST https://app-dqs7nnc9k54x-api-NLZ133Rnwr29-gateway.appmiaoda.com/precious-metal/inter/price` |
...
`scripts/call_precious_metal_kline.py`（`--endpoint inter-price`）。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-NLZ133Rnwr29-gateway.appmiaoda.com/precious-metal/inter/price",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
...
| 端点 | `POST https://app-dqs7nnc9k54x-api-2Y00VV8Rkb2Y-gateway.appmiaoda.com/precious-metal/inter/kline` |
...
生成期直接调用已实现为脚本，详见 `SKILL.md` → **生成期用法（Agent 直接调用）** 章节及
`scripts/call_precious_metal_kline.py`（`--endpoint inter-kline`）。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-2Y00VV8Rkb2Y-gateway.appmiaoda.com/precious-metal/inter/kline",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
...
| 端点 | `POST https://app-dqs7nnc9k54x-api-nYWNRRkexgKL-gateway.appmiaoda.com/precious-metal/inter/contract` |
...
生成期直接调用已实现为脚本，详见 `SKILL.md` → **生成期用法（Agent 直接调用）** 章节及
`scripts/call_precious_metal_kline.py`（`--endpoint inter-contract`）。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-nYWNRRkexgKL-gateway.appmiaoda.com/precious-metal/inter/contract",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
