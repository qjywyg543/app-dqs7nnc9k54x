...
| Third-part Domain | `app-dqs7nnc9k54x-api-ra5Err8G2Rla-gateway.appmiaoda.com` |
| 计费 | 按次计费 |
...
| 端点 | `POST https://app-dqs7nnc9k54x-api-ra5Err8G2Rla-gateway.appmiaoda.com/precious-metal/domestic/price` |
...
生成期直接调用已实现为脚本，详见 `SKILL.md` → **生成期用法（Agent 直接调用）** 章节及
`scripts/call_precious_metal_kline.py`（`--endpoint domestic-price`）。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-ra5Err8G2Rla-gateway.appmiaoda.com/precious-metal/domestic/price",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
...
| 端点 | `POST https://app-dqs7nnc9k54x-api-rLobRR63mpd9-gateway.appmiaoda.com/precious-metal/domestic/kline` |
...
生成期直接调用已实现为脚本，详见 `SKILL.md` → **生成期用法（Agent 直接调用）** 章节及
`scripts/call_precious_metal_kline.py`（`--endpoint domestic-kline`）。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-rLobRR63mpd9-gateway.appmiaoda.com/precious-metal/domestic/kline",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
...
| 端点 | `POST https://app-dqs7nnc9k54x-api-DY8Mnnl0GGAa-gateway.appmiaoda.com/precious-metal/domestic/contract` |
...
生成期直接调用已实现为脚本，详见 `SKILL.md` → **生成期用法（Agent 直接调用）** 章节及
`scripts/call_precious_metal_kline.py`（`--endpoint domestic-contract`）。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-DY8Mnnl0GGAa-gateway.appmiaoda.com/precious-metal/domestic/contract",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
...
- **调用注意**：本 plugin 单次调用即计费，避免频繁轮询造成不必要费用。
