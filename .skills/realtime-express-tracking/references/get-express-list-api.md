...
| Endpoint | `GET https://app-dqs7nnc9k54x-api-V9gDz8wo0V5L-gateway.appmiaoda.com/getExpressList` |
| Auth 模式 | `platform_managed`（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer ${apiKey}` |
| Content-Type | `application/json` |
| third_part_domain | `app-dqs7nnc9k54x-api-V9gDz8wo0V5L-gateway.appmiaoda.com` |
| 计费 | 启用，按次计费 |
...
脚本内部负责读取 `INTEGRATIONS_API_KEY`、拼接 `type` 查询参数、发送请求并输出 `result` 字段。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
serve(async (req: Request): Promise<Response> => {
  ...
  // --- 调用上游 API ---
  const url = new URL(
    "https://app-dqs7nnc9k54x-api-V9gDz8wo0V5L-gateway.appmiaoda.com/getExpressList"
  );
  if (type) {
...
- **计费**：此接口启用计费，按次计费。建议缓存快递公司列表（该数据变动频率低），避免不必要的重复调用。
