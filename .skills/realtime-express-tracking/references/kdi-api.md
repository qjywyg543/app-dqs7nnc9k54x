...
| Endpoint | `GET https://app-dqs7nnc9k54x-api-AalZz7v4QEGL-gateway.appmiaoda.com/kdi` |
| Auth 模式 | `platform_managed`（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer ${apiKey}` |
| Content-Type | `application/json;charset=UTF-8` |
| third_part_domain | `app-dqs7nnc9k54x-api-AalZz7v4QEGL-gateway.appmiaoda.com` |
...
```bash
python3 <skill-path>/scripts/query_tracking.py --no "780098068058" --type ZTO
```

顺丰单号需附加收件人/寄件人手机号后四位：

```bash
python3 <skill-path>/scripts/query_tracking.py --no "123456789:1234" --type SF
```

脚本内部负责读取 `INTEGRATIONS_API_KEY`、拼接 `no`/`type` 查询参数、发送请求并输出 `result` 字段。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
serve(async (req: Request): Promise<Response> => {
  ...
  // --- 调用上游 API ---
  const url = new URL(
    "https://app-dqs7nnc9k54x-api-AalZz7v4QEGL-gateway.appmiaoda.com/kdi"
  );
