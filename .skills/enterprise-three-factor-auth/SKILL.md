...
- **Endpoint**: `GET https://app-dqs7nnc9k54x-api-Aa2PZnjE80BL-gateway.appmiaoda.com/company_three/get`
...
直接调用 `scripts/verify_enterprise_three_factor.py`，脚本从环境变量 `INTEGRATIONS_API_KEY` 读取密钥，并设置 `X-Gateway-Authorization: Bearer <key>`。

Bash 工具调用时 timeout 必须设置为 600000ms（600 秒）。

```bash
python3 scripts/verify_enterprise_three_factor.py --company-name "示例科技有限公司" --credit-no "91110000000000000X" --legal-person "张三"
```

成功时 stdout 输出一行 JSON：

```json
{"success":true,"code":200,"msg":"成功","data":{"order_no":"...","result":0,"desc":"一致"}}
```
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-dqs7nnc9k54x-api-Aa2PZnjE80BL-gateway.appmiaoda.com/company_three/get?${params.toString()}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json;charset=UTF-8",
...
- **third_part_domain**: `app-dqs7nnc9k54x-api-Aa2PZnjE80BL-gateway.appmiaoda.com`
