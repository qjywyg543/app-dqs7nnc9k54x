...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-oYA6ZGjReooa-gateway.appmiaoda.com/rest/2.0/ocr/v1/doc_convert/get_request_result` |
| Content-Type | `application/x-www-form-urlencoded` |
| Auth 模式 | `platform_managed`（`traefik: true`） |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| third_part_domain | `app-dqs7nnc9k54x-api-oYA6ZGjReooa-gateway.appmiaoda.com` |
| 计费 | **不计费**，按需轮询 |
...
生成期（Agent 直接调用）已改为脚本调用方式，不再使用本节 API 的内联调用代码。查询与轮询逻辑（含 `ret_code = 3` 完成态处理）已固化进 `scripts/generate_doc_convert.py` 和 `scripts/query_doc_convert.py`，具体命令示例见 `SKILL.md` 的"生成期用法（Agent 直接调用）"一节。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-oYA6ZGjReooa-gateway.appmiaoda.com/rest/2.0/ocr/v1/doc_convert/get_request_result",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
