...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-rY7JZ6jqrneL-gateway.appmiaoda.com/rest/2.0/ocr/v1/doc_convert/request` |
| Content-Type | `application/x-www-form-urlencoded` |
| Auth 模式 | `platform_managed`（`traefik: true`） |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| third_part_domain | `app-dqs7nnc9k54x-api-rY7JZ6jqrneL-gateway.appmiaoda.com` |
| 计费 | 启用计费，按调用次数计费；查询接口不计费 |
...
生成期（Agent 直接调用）已改为脚本调用方式，不再使用本节 API 的内联调用代码。提交与轮询逻辑已固化进 `scripts/generate_doc_convert.py`，具体命令示例见 `SKILL.md` 的"生成期用法（Agent 直接调用）"一节。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-rY7JZ6jqrneL-gateway.appmiaoda.com/rest/2.0/ocr/v1/doc_convert/request",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
