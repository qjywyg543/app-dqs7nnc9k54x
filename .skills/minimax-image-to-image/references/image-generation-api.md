...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-6LeBzWJjy3QY-gateway.appmiaoda.com/v1/image_generation` |
| 认证方式 | `platform_managed`（`X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}`） |
| Content-Type | `application/json` |
| third_part_domain | `app-dqs7nnc9k54x-api-6LeBzWJjy3QY-gateway.appmiaoda.com` |
| stream | 否 |
...
生成期调用已收敛到脚本，避免模型现场生成请求代码、Base64 编码和下载命令。
...
图生图（本地参考图会在脚本内编码为 Base64 Data URL）：
...
多图生成使用 `--output-dir`；公网参考图可用 `--reference-url`。

脚本内部处理：

1. 读取 `INTEGRATIONS_API_KEY`，通过 `X-Gateway-Authorization: Bearer <key>` 调用上游
2. 将 `--reference-image` 本地文件编码为 Data URL，放入 `subject_reference`
3. 请求 `response_format=url`
4. 从响应 `data.image_urls` 获取临时 URL
5. 立即下载到 `--output` 或 `--output-dir`
6. stdout 输出一行 JSON
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-6LeBzWJjy3QY-gateway.appmiaoda.com/v1/image_generation",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
4. **计费**：按调用次数计费，具体以平台实际配置为准，避免不必要的批量生成。
5. **错误处理**：
