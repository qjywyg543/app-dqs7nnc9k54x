...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-W9z3M74x6ZNL-gateway.appmiaoda.com/v1/code/send_message` |
| 认证模式 | `platform_managed` |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/json` |
| third_part_domain | `app-dqs7nnc9k54x-api-W9z3M74x6ZNL-gateway.appmiaoda.com` |
| 说明 | 用于向指定手机号发送短信验证码 |
...
生成期调用已改为脚本方式，模型只需拼命令并执行即可。

```bash
python3 <skill-path>/scripts/send_sms_code.py --mobile 13800138000 [--session-id <已有 sessionId>]
```

脚本会读取 `INTEGRATIONS_API_KEY`，成功时 stdout 输出一行 JSON，包含 `sessionId`（后续验证时必须传入）。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-W9z3M74x6ZNL-gateway.appmiaoda.com/v1/code/send_message",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
