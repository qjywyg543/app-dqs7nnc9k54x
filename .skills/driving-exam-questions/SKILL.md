...
- **Endpoint**: `POST https://app-dqs7nnc9k54x-api-o9wN0mk1DAPa-gateway.appmiaoda.com/driverexam/query`
...
直接调用 `scripts/query_driving_exam_questions.py`，脚本从环境变量 `INTEGRATIONS_API_KEY` 读取密钥，并设置 `X-Gateway-Authorization: Bearer <key>`。

Bash 工具调用时 timeout 必须设置为 600000ms（600 秒）。

```bash
python3 scripts/query_driving_exam_questions.py --type C1 --subject 1 --pagesize 5 --sort rand
python3 scripts/query_driving_exam_questions.py --type C1 --subject 4 --chapter 2 --pagenum 1 --pagesize 10
```

成功时 stdout 输出一行 JSON：

```json
{"status":0,"msg":"ok","result":{"total":"1000","pagenum":"1","pagesize":"5","list":[{"question":"...","answer":"A","explain":"..."}]}}
```
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-dqs7nnc9k54x-api-o9wN0mk1DAPa-gateway.appmiaoda.com/driverexam/query?${params.toString()}`,
    {
      method: "POST",
      headers: {
...
- **计费**：按调用次数计费，请避免不必要的重复调用；`pagesize` 建议根据实际需求合理设置，避免过度拉取。
