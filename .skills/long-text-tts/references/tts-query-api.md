...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-Q9KWZ2jy8W09-gateway.appmiaoda.com/rpc/2.0/tts/v1/query` |
| 生成期 URL | `https://app-dqs7nnc9k54x-api-Q9KWZ2jy8W09-gateway.appmiaoda.com/rpc/2.0/tts/v1/query` |
| Auth 模式 | `platform_managed`（traefik: true） |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/json` |
| Third-party domain | `app-dqs7nnc9k54x-api-Q9KWZ2jy8W09-gateway.appmiaoda.com` |
| 计费 | 不计费 |
...
生成期（Agent 直接调用）已改为脚本调用方式，不再使用本节 API 的内联调用代码。查询逻辑（含 QPS 超限 `error_code: 18` 的退避重试处理）已固化进 `scripts/generate_tts.py`（提交+轮询）和 `scripts/query_tts.py`（仅轮询）。具体命令示例见 `SKILL.md` 的"生成期用法（Agent 直接调用）"一节。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-Q9KWZ2jy8W09-gateway.appmiaoda.com/rpc/2.0/tts/v1/query",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
- **计费**：`tts-query` 本身不计费，但需配合 `tts-create`（按调用次数计费）使用
- **轮询实现 CRITICAL**：
