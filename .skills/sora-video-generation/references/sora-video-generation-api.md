...
|--------|-------|----------|--------|--------------|
| `api-Xa6Jew6JjAqa` | Create Video | `https://app-dqs7nnc9k54x-api-Xa6Jew6JjAqa-gateway.appmiaoda.com/openai/v1/videos` | POST | multipart/form-data |
| `api-M9v0w87KjxoY` | Query Status | `https://app-dqs7nnc9k54x-api-M9v0w87KjxoY-gateway.appmiaoda.com/query` | POST | application/json |
| `api-W9z3qro1AVZL` | Video from Reference | `https://app-dqs7nnc9k54x-api-W9z3qro1AVZL-gateway.appmiaoda.com/openai/v1/videos` | POST | multipart/form-data |
| `api-M9v0wP10kQjY` | Remix Video | `https://app-dqs7nnc9k54x-api-M9v0wP10kQjY-gateway.appmiaoda.com/openai/v1/videos/remix` | POST | application/json |
...
**third_part_domain:** `app-dqs7nnc9k54x-api-Xa6Jew6JjAqa-gateway.appmiaoda.com`
...
**third_part_domain:** `app-dqs7nnc9k54x-api-M9v0w87KjxoY-gateway.appmiaoda.com`
...
生成期（Agent 直接调用）已改为脚本调用方式，不再使用本节 API 的内联调用代码。具体命令示例见 `SKILL.md` 的"生成期用法（Agent 直接调用）"一节（对应脚本 `scripts/generate_sora_video.py` 和 `scripts/query_sora_video.py`）。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-Xa6Jew6JjAqa-gateway.appmiaoda.com/openai/v1/videos",
    {
      method: "POST",
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-W9z3qro1AVZL-gateway.appmiaoda.com/openai/v1/videos",
    {
      method: "POST",
      headers: { "X-Gateway-Authorization": `Bearer ${apiKey}` },
      body: form,
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-M9v0w87KjxoY-gateway.appmiaoda.com/query",
    {
      method: "POST",
      headers: {
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-M9v0wP10kQjY-gateway.appmiaoda.com/openai/v1/videos/remix",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
| 接口 | 计费状态 | 说明 |
|------|----------|------|
| Create Video | 启用 | 避免重复发起任务 |
| Query Status | 不计费 | 可频繁轮询 |
| Video from Reference | 启用 | 用于图生视频 |
| Remix Video | 启用 | 用于视频 Remix |
...
- **文件大小**：≤ 10 MB；超出时应在生成期 / Edge Function 侧提前报错，避免无效调用。
