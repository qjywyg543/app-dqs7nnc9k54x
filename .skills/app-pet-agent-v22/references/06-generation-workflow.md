...
> **正确顺序**：① 分析目标应用 → ② 输出制作确认单 → ③ 用户确认角色/性格/状态/互动方向 → ④ 生成 canonical base → ⑤ 以 canonical base 为唯一身份参考生成状态 Sprite Sheet → ⑥ 素材处理、透明背景与统一几何规范化 → ⑦ geometry QA / contact sheet / motion preview / Character Identity QA / Cross-State QA / Application Visual Integration QA → ⑧ 正式资产通过 QA 后才接入代码 → ⑨ lint / type-check / build / 应用内预览。

### Step 1: 确定角色基准

**输入**：用户参考图或文字描述 + Application Visual System（应用画风参考）。

**输出**：`canonical-base-pet.png`。
