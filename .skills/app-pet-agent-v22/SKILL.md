---
name: app-pet-agent-v22
description: 为秒哒 Web/H5 应用设计并接入能够感知业务状态、支持拖拽和轻量互动的陪伴式应用宠物。适用于用户要求添加应用宠物、桌宠、状态宠物、宠物 Agent、业务状态反馈角色，或升级和检查既有应用宠物实现时。
license: MIT
---

# 应用宠物 Agent Skill V2.2
...
- **制作方案确认单强制确认**：在生成任何素材或修改项目前，必须输出确认单并等待用户确认。用户确认后自动执行完整流程，**不在 canonical base 之后再次要求确认**。
- **性格配置纳入确认单**：确认单必须包含 archetype / energy / warmth / doubleTap 方向，由用户确认或覆盖 Skill 推荐。
- **正式资产必须经过生成流程**：正式宠物主体必须经过角色图像生成和 Sprite Sheet 生产流程。**禁止**用手写 SVG、CSS 几何图形、纯色圆点、图标或临时占位角色代替正式生成的宠物；禁止因图片生成或 Sprite 处理失败就改成手绘 SVG 并宣称完成；禁止跳过 canonical base 直接分别生成互不一致的状态；禁止先写接入组件再用占位资产完成接入。
...
### Step 4: 生成 canonical base

- 基于用户确认的角色来源 + 应用画风参考，生成 canonical base pet。
- canonical base 是内部生产与 QA 基准，**不是新的默认人工阻断点**；用户确认制作方案后自动继续，不要求二次确认。
- 详见 `references/06-generation-workflow.md` §1。

### Step 5: 以 canonical base 为唯一身份参考生成状态 Sprite Sheet

- 推荐生成 idle / working / success / remind 四种状态。
- 不同状态必须共享单帧尺寸、scale、bottom-center anchor，避免跨状态跳动。
- 详见 `references/06-generation-workflow.md` §2、`references/11-sprite-geometry.md` 。
