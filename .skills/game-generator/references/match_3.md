...
* **Await ALL tweens, not just the last** — resolve only when EVERY tween completes (counter or `Promise.all`), never `tweens[last].on('complete', resolve)`. Concurrent tweens finish out of push-order, so the last-pushed one may fire early → cascade rescans mid-animation and `inputLocked` can stick. Applies to `removeTiles` / `applyGravity` / `fillEmpty` / swap animations alike.
```typescript
async applyGravity(): Promise<void> {
  return new Promise(resolve => {
    ...
    let done = 0;
    tweens.forEach(t => t.on('complete', () => { if (++done === tweens.length) resolve(); }));
  });
}
```
</GRAVITY_AND_FILL>
...
* **Absolute scale destroys `setDisplaySize` fit (heterogeneous assets)** — if tile textures have non-uniform native sizes, cell-fit is owned SOLELY by `setDisplaySize(TILE_SIZE*k)`. NEVER set/tween a literal `scale→N` as juice — it renders `native_px×N` and overflows the cell, stacking neighbors into one blob. Use alpha/tint/overlay-ring for pop-in, highlight, and hint; only scale-to-0 on removal is safe (uniform shrink ≤ fit).
</SCALE_TWEEN_PITFALLS>

<VISUAL_POLISH>
* **Tile pop-in**: spawn new tiles at `scale:0`, tween to `scale:1` with `Back.Out` ease. **Only if all tile textures share one native size**; for heterogeneous assets sized via `setDisplaySize`, pop-in with `alpha:0→1` instead (see SCALE_TWEEN_PITFALLS).
...
<PERSISTENT_DISPATCH_OWNERSHIP>
Symptom: level 1 plays fine, then EVERY next level freezes (HUD updates but board is locked/blank). Cause is WHO owns the cross-scene dispatcher.
* **`this.scene.start()` inside a scene routes through ScenePlugin, which `queueOp('stop', self)` FIRST, then starts the target** — i.e. it SHUTS DOWN the caller. `SceneManager.start` (via `gameRef.current.scene.start(...)`) does NOT touch the caller. Same call shape, opposite side-effect.
* **NEVER register a long-lived cross-scene listener inside a one-shot scene** (e.g. `EventBus.on('start-game', …)` in `Preloader.create()`). On first fire the host scene shuts down, and the orphaned closure driving the next level is not guaranteed to survive.
* **Own persistent dispatch from an immortal host** — register `'start-game'` and level transitions in `src/App.tsx` `useEffect`, and mirror EVERY key with `EventBus.off(...)` in cleanup. External-emitter listeners are NOT cleaned by scene shutdown.
</PERSISTENT_DISPATCH_OWNERSHIP>
</GAME>
