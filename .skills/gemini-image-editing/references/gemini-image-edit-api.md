...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-o9wN0AExZQ8a-gateway.appmiaoda.com/v1beta/models/gemini-3.1-flash-image-preview:generateContent` |
| Content-Type | `application/json` |
| 认证模式 | `platform_managed` |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| 第三方域名 | `app-dqs7nnc9k54x-api-o9wN0AExZQ8a-gateway.appmiaoda.com` |
| 流式响应 | 否 |
| 计费 | 按调用次数计费 |
...
生成期调用已收敛到脚本，避免图片 Base64 进入模型上下文。
...
1. 读取本地图片并自动推断 MIME 类型（也可用 `--mime-type` 覆盖）
2. 将图片编码为 Base64，组装 `contents[0].parts` 请求体
3. 使用 `INTEGRATIONS_API_KEY` 通过 `X-Gateway-Authorization: Bearer <key>` 调用上游
4. 从响应 `candidates[0].content.parts` 中提取 `inlineData.data`
5. 解码图片并写入 `--output`
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-o9wN0AExZQ8a-gateway.appmiaoda.com/v1beta/models/gemini-3.1-flash-image-preview:generateContent",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
