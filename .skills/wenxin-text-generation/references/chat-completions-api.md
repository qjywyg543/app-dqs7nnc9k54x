...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-zYkZz8qovQ1L-gateway.appmiaoda.com/v2/chat/completions` |
| Content-Type | `application/json` |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| 认证模式 | `platform_managed`（密钥由平台注入，读取 `INTEGRATIONS_API_KEY`） |
| Third-party Domain | `app-dqs7nnc9k54x-api-zYkZz8qovQ1L-gateway.appmiaoda.com` |
| 响应模式 | SSE 流式 |
...
脚本内部负责读取 `INTEGRATIONS_API_KEY`、发送请求、解析 SSE 并聚合 `choices[].delta.content`。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-zYkZz8qovQ1L-gateway.appmiaoda.com/v2/chat/completions",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
- `need_count_calls: true`，避免不必要的重复调用。
