...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-l9nZz8ro7Bl9-gateway.appmiaoda.com/rest/2.0/face/v3/faceset/user/add` |
| Auth 模式 | `platform_managed` |
| Auth Header | `X-Gateway-Authorization: Bearer <INTEGRATIONS_API_KEY>` |
| Content-Type | `application/json` |
| third_part_domain | `app-dqs7nnc9k54x-api-l9nZz8ro7Bl9-gateway.appmiaoda.com` |
| 计费 | 是，按调用次数计费 |
...
```bash
python3 <skill-path>/scripts/call_user_add.py --image /path/to/face.jpg --user-id user_001 [--user-info "备注信息"] [--quality-control NORMAL] [--liveness-control NORMAL] [--action-type APPEND]

# 使用已有 FACE_TOKEN
python3 <skill-path>/scripts/call_user_add.py --face-token <face_token> --user-id user_001
```

脚本成功时 stdout 输出一行 JSON（即上游接口原始响应，含 `result.face_token`、`result.location` 等字段）。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-l9nZz8ro7Bl9-gateway.appmiaoda.com/rest/2.0/face/v3/faceset/user/add",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
