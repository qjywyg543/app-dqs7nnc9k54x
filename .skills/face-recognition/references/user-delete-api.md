...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-ELbWz8OmewKY-gateway.appmiaoda.com/rest/2.0/face/v3/faceset/user/delete` |
| Auth 模式 | `platform_managed` |
| Auth Header | `X-Gateway-Authorization: Bearer <INTEGRATIONS_API_KEY>` |
| Content-Type | `application/json` |
| third_part_domain | `app-dqs7nnc9k54x-api-ELbWz8OmewKY-gateway.appmiaoda.com` |
| 计费 | 否 |
...
```bash
python3 <skill-path>/scripts/call_user_delete.py --user-id user_001
```

脚本成功时 stdout 输出一行 JSON（即上游接口原始响应，含 `error_code`、`log_id` 等字段）。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-ELbWz8OmewKY-gateway.appmiaoda.com/rest/2.0/face/v3/faceset/user/delete",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
