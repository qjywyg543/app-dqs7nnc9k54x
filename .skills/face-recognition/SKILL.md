...
本 Skill 封装百度 AI 人脸识别平台（`app-dqs7nnc9k54x-api-l9nZz8ro7Bl9-gateway.appmiaoda.com`）的五个核心接口，通过 API Gateway 统一鉴权（platform_managed）对外暴露。

| 接口 | Endpoint | Method | 是否计费 |
|------|----------|--------|---------|
| 人脸注册 | `POST /rest/2.0/face/v3/faceset/user/add` | POST | 是 |
| 人脸搜索 | `POST /rest/2.0/face/v3/search` | POST | 是 |
...
生成期调用必须使用 `scripts/` 下对应脚本；脚本内部读取 `INTEGRATIONS_API_KEY`、完成 base64/json 请求，并输出单行 JSON。各接口的完整参数详见对应 references 文件：

- 人脸注册：`python3 scripts/call_user_add.py --image /path/to/face.jpg --user-id user_001`，详见 `references/user-add-api.md`
- 人脸搜索：`python3 scripts/call_face_search.py --image /path/to/face.jpg`，详见 `references/face-search-api.md`
- 用户信息查询：`python3 scripts/call_user_get.py --user-id user_001`，详见 `references/user-get-api.md`
- 用户列表获取：`python3 scripts/call_group_getusers.py --start 0 --length 100`，详见 `references/group-getusers-api.md`
- 删除用户：`python3 scripts/call_user_delete.py --user-id user_001`，详见 `references/user-delete-api.md`
