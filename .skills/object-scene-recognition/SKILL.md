...
生成期直接使用脚本调用上游 API，适用于生成阶段的批处理或数据分析场景，避免图片 Base64 进入模型上下文。

```bash
python3 <skill-path>/scripts/recognize_scene.py --image /path/to/image.jpg
```

也可以使用图片 URL，并按需返回百科信息：

```bash
python3 <skill-path>/scripts/recognize_scene.py --url https://example.com/image.jpg --baike-num 3
```

脚本内部会读取本地图片并完成 Base64 编码，使用 `INTEGRATIONS_API_KEY` 通过 `X-Gateway-Authorization` 调用上游。成功时 stdout 输出一行识别结果 JSON（`result` 数组等字段）。
