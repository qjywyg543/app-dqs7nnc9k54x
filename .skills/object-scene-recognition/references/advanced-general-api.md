...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-zYm4zKQoePjL-gateway.appmiaoda.com/rest/2.0/image-classify/v2/advanced_general` |
| 生成期 Endpoint（含 API ID 前缀） | `POST https://app-dqs7nnc9k54x-api-zYm4zKQoePjL-gateway.appmiaoda.com/rest/2.0/image-classify/v2/advanced_general` |
| Auth 模式 | `platform_managed` |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/x-www-form-urlencoded` |
| third_part_domain | `app-dqs7nnc9k54x-api-zYm4zKQoePjL-gateway.appmiaoda.com` |
| 流式响应 | 否 |
...
生成期调用请使用内置脚本，脚本会从环境变量读取 INTEGRATIONS_API_KEY，避免图片 Base64 进入模型上下文。

```bash
python3 <skill-path>/scripts/recognize_scene.py --image /path/to/image.jpg
```

也可以传入 URL，并按需返回百科信息：

```bash
python3 <skill-path>/scripts/recognize_scene.py --url https://example.com/image.jpg --baike-num 3
```
...
| `--url` | 图片 URL，与 `--image` 二选一 |
| `--baike-num` | 返回百科信息数量，0-5 |

脚本内部读取 `INTEGRATIONS_API_KEY`，使用 `X-Gateway-Authorization: Bearer <key>` 调用上游。成功时 stdout 输出一行识别 JSON。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-zYm4zKQoePjL-gateway.appmiaoda.com/rest/2.0/image-classify/v2/advanced_general",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
...
- **调用注意**：按调用次数计费，请避免重复调用同一张图片，建议在前端缓存识别结果。
