...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-Aa2PqMJnJGwL-gateway.appmiaoda.com/v1/text/chatcompletion_v2` |
| Third-party Domain | `app-dqs7nnc9k54x-api-Aa2PqMJnJGwL-gateway.appmiaoda.com` |
...
```bash
python3 <skill-path>/scripts/call_chat_completion.py --prompt "写一段产品发布会开场白"
```

脚本内部负责读取 `INTEGRATIONS_API_KEY`、发送请求，并在 `--stream` 模式下解析 SSE 聚合 `choices[].delta.content`。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-Aa2PqMJnJGwL-gateway.appmiaoda.com/v1/text/chatcompletion_v2",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
3. **计费**：当前免费，但 `need_count_calls: true` 表示调用次数仍会统计，避免不必要的重复调用。
