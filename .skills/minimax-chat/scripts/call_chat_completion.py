#!/usr/bin/env python3
"""Call MiniMax-M2.5 chat completion. Supports non-streaming and streaming (SSE) modes."""
...
ENDPOINT = "https://app-dqs7nnc9k54x-api-Aa2PqMJnJGwL-gateway.appmiaoda.com/v1/text/chatcompletion_v2"
MODEL = "MiniMax-M2.5"
...
def main():
    """入口：解析参数，调用聊天接口并输出结果 JSON。"""
    parser = argparse.ArgumentParser(description="Call MiniMax chat completion and print one JSON result.")
    ...
    body = {"model": MODEL, "messages": messages, "stream": args.stream}
    ...
    try:
        with urllib.request.urlopen(request, timeout=600) as response:
            if args.stream:
                ...
                result = {"status": "succeed", "content": "".join(content_parts), "finish_reason": finish_reason}
            else:
                ...
                result = {"status": "succeed", "content": content, "raw": data}
