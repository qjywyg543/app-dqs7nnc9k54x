...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-DYJwo27V8Qya-gateway.appmiaoda.com/v2/ai_search/chat/completions` |
| 认证模式 | platform_managed |
| Auth Header | `X-Gateway-Authorization: Bearer <INTEGRATIONS_API_KEY>` |
| Content-Type | `application/json` |
| 响应类型 | SSE 流式（text/event-stream） |
| third_part_domain | `app-dqs7nnc9k54x-api-DYJwo27V8Qya-gateway.appmiaoda.com` |
...
生成期（Agent 直接调用）已改为脚本调用方式，不再使用本节 API 的内联 SSE 读取代码。具体命令示例见 `SKILL.md` 的“生成期用法（Agent 直接调用）”一节（对应脚本 `scripts/call_ai_search.py`）。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-DYJwo27V8Qya-gateway.appmiaoda.com/v2/ai_search/chat/completions",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
