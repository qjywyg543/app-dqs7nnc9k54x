...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-DLEO7Vjd8Qea-gateway.appmiaoda.com/rest/2.0/ocr/v1/air_ticket` |
| Auth 模式 | `platform_managed` |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/x-www-form-urlencoded` |
| Third-party Domain | `app-dqs7nnc9k54x-api-DLEO7Vjd8Qea-gateway.appmiaoda.com` |
| 计费 | 启用；按调用次数计费 |
...
# PDF / OFD 文件
python3 <skill-path>/scripts/call_air_ticket_ocr.py --pdf-file /path/to/doc.pdf [--pdf-file-num 1]
python3 <skill-path>/scripts/call_air_ticket_ocr.py --ofd-file /path/to/doc.ofd [--ofd-file-num 1]
```

脚本成功时 stdout 输出一行 JSON（即上游接口原始响应，含 `words_result` / `words_result_num` 等字段）。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-DLEO7Vjd8Qea-gateway.appmiaoda.com/rest/2.0/ocr/v1/air_ticket",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
