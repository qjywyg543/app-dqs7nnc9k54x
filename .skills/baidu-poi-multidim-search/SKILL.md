...
| 属性 | 值 |
|------|-----|
| Endpoint | `GET https://app-dqs7nnc9k54x-api-zYkZXBBNPe8L-gateway.appmiaoda.com/api_place_pro/v1/region` |
| 原始 API scheme | 原始 APIDOC 端点为 `http://app-dqs7nnc9k54x-api-zYkZXBBNPe8L-gateway.appmiaoda.com/api_place_pro/v1/region`（HTTP），gateway 已将其升级为 HTTPS 代理访问，调用时统一使用上方 HTTPS 地址即可 |
...
生成期直接调用脚本 `scripts/call_baidu_poi_multidim_search.py`，脚本内完成密钥读取、请求、结果解析。**Bash 工具调用超时必须设置为 `600000ms`（600 秒）。**

```bash
# 基础检索
python3 scripts/call_baidu_poi_multidim_search.py --query "宠物友好餐厅" --region "北京市"

# 详细信息 + 区域限定 + 按评分降序
python3 scripts/call_baidu_poi_multidim_search.py --query "宠物友好餐厅" --region "郑州" \
  --region-limit true --scope 2 --page-num 0 --page-size 10 \
  --filter "industry_type:cater sort_name:overall_rating sort_rule:1" --extensions-adcode true

# 距离排序（同时传 center 和 filter 中的距离排序参数）
python3 scripts/call_baidu_poi_multidim_search.py --query "银行" --region "北京市海淀区" \
  --center "39.98,116.31" --filter "sort_name:distance sort_rule:0"
```

成功时 stdout 输出一行 JSON：`{"status":"succeed","result":{...上游原始响应...}}`。参数细节参见下方「参数说明」。生成期调用实现以 `scripts/call_baidu_poi_multidim_search.py` 为准。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-dqs7nnc9k54x-api-zYkZXBBNPe8L-gateway.appmiaoda.com/api_place_pro/v1/region?${params.toString()}`,
    {
      method: "GET",
      headers: {
