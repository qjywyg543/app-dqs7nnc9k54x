...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-k93RZBjP0zqa-gateway.appmiaoda.com/rest/2.0/ocr/v1/idcard` |
| Content-Type | `application/x-www-form-urlencoded` |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| 认证模式 | `platform_managed` |
| third_part_domain | `app-dqs7nnc9k54x-api-k93RZBjP0zqa-gateway.appmiaoda.com` |
...
生成期调用已收敛到脚本，避免身份证图片 Base64 进入模型上下文。
...
```bash
python3 <skill-path>/scripts/recognize_idcard.py --url https://example.com/idcard-back.jpg --side back --detect-risk --detect-quality
```
...
| 参数 | 说明 |
|------|------|
| `--image` | 本地身份证图片路径，与 `--url` 二选一 |
| `--url` | 图片 URL，与 `--image` 二选一 |
| `--side` | 必填，`front` 或 `back` |
| `--detect-ps` | 开启 PS 检测 |
| `--detect-risk` | 开启风险检测 |
| `--detect-quality` | 开启质量检测 |
| `--detect-direction` | 开启方向检测 |

脚本内部读取 `INTEGRATIONS_API_KEY`，使用 `X-Gateway-Authorization: Bearer <key>` 调用上游。成功时 stdout 输出一行 OCR JSON。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-k93RZBjP0zqa-gateway.appmiaoda.com/rest/2.0/ocr/v1/idcard",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
...
- **第三方域名**：实际请求转发至 `app-dqs7nnc9k54x-api-k93RZBjP0zqa-gateway.appmiaoda.com`，网关代理后客户端无需感知。
