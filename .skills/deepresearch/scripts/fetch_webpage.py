...
Failure does NOT die; status is recorded and returned. Only die on missing API key
or invalid CLI arguments.
"""
...
def save_content(url, content):
    """把正文写到 fetch_<...>.md（纯 markdown），返回相对项目根的路径。"""
    os.makedirs(RESEARCH_DIR, exist_ok=True)
    path = os.path.join(RESEARCH_DIR, url_to_filename(url))
    with open(path, "w", encoding="utf-8") as f:
        f.write(f"<!-- url: {url} -->\n\n")
        f.write(content)
    return os.path.relpath(path, DEFAULT_PROJECT_ROOT).replace("\\", "/")
