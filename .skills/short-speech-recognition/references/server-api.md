...
| Endpoint | `https://app-dqs7nnc9k54x-api-Aa2PZnjEw5NL-gateway.appmiaoda.com/server_api` |
| 网关 URL（含 API ID 前缀） | `https://app-dqs7nnc9k54x-api-Aa2PZnjEw5NL-gateway.appmiaoda.com/server_api` |
| Method | POST |
| Content-Type | application/json |
| third_part_domain | `app-dqs7nnc9k54x-api-Aa2PZnjEw5NL-gateway.appmiaoda.com` |
...
```bash
python3 <skill-path>/scripts/recognize_speech.py --file /path/to/audio.wav --format wav --rate 16000
```

脚本内部负责读取 `INTEGRATIONS_API_KEY`、读取本地音频文件并 base64 编码、发送请求并输出识别结果。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
serve(async (req: Request): Promise<Response> => {
  ...
  // --- 调用上游 API ---
  const upstream = await fetch("https://app-dqs7nnc9k54x-api-Aa2PZnjEw5NL-gateway.appmiaoda.com/server_api", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
