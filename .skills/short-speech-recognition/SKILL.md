...
生成期调用已收敛到脚本，模型只需要传本地音频路径，不再现场编写请求代码或把 Base64 放进上下文。

**必须将 Bash 工具的超时时间设置为 600000ms（600 秒）**。

```bash
python3 <skill-path>/scripts/recognize_speech.py --file /path/to/audio.wav --format wav --rate 16000 --cuid user-123
```

m4a 音频示例：

```bash
python3 <skill-path>/scripts/recognize_speech.py --file /path/to/audio.m4a --format m4a --rate 16000
```

成功时 stdout 输出一行 JSON：

```json
{"status":"succeed","data":{"err_no":0,"err_msg":"success.","result":["北京科技馆"]}}
```
