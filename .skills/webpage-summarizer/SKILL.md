...
- **Endpoint**: `POST https://app-dqs7nnc9k54x-api-DY8MNXjBpKAa-gateway.appmiaoda.com/v2/components/c-wf-e1bc471f-1d33-4df1-ab42-87800e89c1ad`
...
```bash
python3 scripts/call_webpage_summarizer.py --query "请帮我总结这篇文章的主要内容" --url "https://example.com/article"
```

Bash 工具调用时 timeout 必须设置为 `600000ms（600 秒）`（注意：上游 apiClient 侧超时仍为 60s，`600000ms` 是 Bash 工具调用本身的超时余量）。

成功时 stdout 输出一行 JSON：

```json
{"status":"succeed","result":{"output":"网页内容分析：...","raw":{}}}
```

生成期调用实现以 `scripts/call_webpage_summarizer.py` 为准。
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    upstream = await fetch(
      "https://app-dqs7nnc9k54x-api-DY8MNXjBpKAa-gateway.appmiaoda.com/v2/components/c-wf-e1bc471f-1d33-4df1-ab42-87800e89c1ad",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
- **说明**: 此插件当前不启用计费，但调用次数会被统计，避免不必要的重复调用。
