...
| Endpoint | `GET https://app-dqs7nnc9k54x-api-baBwZEjbe1X9-gateway.appmiaoda.com/reverse_geocoding/v3` |
| 认证模式 | `platform_managed`（`traefik: true`） |
| Auth Header | `X-Gateway-Authorization: Bearer <INTEGRATIONS_API_KEY>` |
| Content-Type | `application/x-www-form-urlencoded`（GET 请求参数附在 URL 上） |
| third_part_domain | `app-dqs7nnc9k54x-api-baBwZEjbe1X9-gateway.appmiaoda.com` |
...
```bash
python3 <skill-path>/scripts/call_geocoding.py reverse --location "31.225696563611,121.49884033194"
```

脚本内部负责读取 `INTEGRATIONS_API_KEY`、发送请求并输出逆地理编码结果。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-dqs7nnc9k54x-api-baBwZEjbe1X9-gateway.appmiaoda.com/reverse_geocoding/v3?${params.toString()}`,
    {
      method: "GET",
      headers: {
