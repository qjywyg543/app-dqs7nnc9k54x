...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-zYkZz8qoKDdL-gateway.appmiaoda.com/rest/2.0/image-classify/v1/image-understanding/get-result` |
| 生成期 URL（含 API ID） | `https://app-dqs7nnc9k54x-api-zYkZz8qoKDdL-gateway.appmiaoda.com/rest/2.0/image-classify/v1/image-understanding/get-result` |
| Content-Type | `application/json` |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| 认证模式 | `platform_managed` |
| third_part_domain | `app-dqs7nnc9k54x-api-zYkZz8qoKDdL-gateway.appmiaoda.com` |
| 计费 | 不启用（查询接口免费） |
...
生成期调用已改为脚本方式，`generate_image_understanding.py` 已内置提交 + 轮询。仅当它返回 `{"status":"processing","task_id":"..."}` 时，才用本接口对应的脚本接力轮询（不会重新提交任务）。

```bash
python3 <skill-path>/scripts/query_image_understanding.py --task-id <task_id>
```

成功时 stdout：`{"status":"succeed","task_id":"...","description":"..."}`
仍未完成时 stdout：`{"status":"processing","task_id":"..."}`——可继续用同一命令接力。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-zYkZz8qoKDdL-gateway.appmiaoda.com/rest/2.0/image-classify/v1/image-understanding/get-result",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
- **提交接口计费提醒**：提交请求（`image-understanding-request`）启用计费，避免因前端 Bug 重复提交。
