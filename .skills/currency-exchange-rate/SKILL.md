...
- **Endpoint**: `POST https://app-dqs7nnc9k54x-api-ELbWz8OmBW5Y-gateway.appmiaoda.com/exchange-rate-v2/single`
...
直接调用 `scripts/query_exchange_rate.py`，脚本从环境变量 `INTEGRATIONS_API_KEY` 读取密钥，并设置 `X-Gateway-Authorization: Bearer <key>`。

Bash 工具调用时 timeout 必须设置为 600000ms（600 秒）。

```bash
python3 scripts/query_exchange_rate.py --from-code CNY
```

成功时 stdout 输出一行 JSON：

```json
{"status":0,"msg":"ok","data":{"name":"人民币","currency":"CNY","list":{"USD":{"rate":"0.1394","name":"美元","updatetime":"2026-01-01 12:00:00"}}}}
```
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-ELbWz8OmBW5Y-gateway.appmiaoda.com/exchange-rate-v2/single",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
...
- **计费**: 该接口启用计费，每次调用均计费，请避免不必要的重复调用（如轮询或频繁刷新）。
