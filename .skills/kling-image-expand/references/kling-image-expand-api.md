...
| third_part_domain | `app-dqs7nnc9k54x-api-Q9KWnzwVQMk9-gateway.appmiaoda.com` |
| 说明 | 图片扩展任务提交接口（`enable_billing: false` 当前未计费） |
...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-Q9KWnzwVQMk9-gateway.appmiaoda.com/v1/images/editing/expand` |
...
| Endpoint | `GET https://app-dqs7nnc9k54x-api-rLobR6vwZJJ9-gateway.appmiaoda.com/v1/images/editing/expand/{task_id}` |
| Content-Type | `application/json` |
...
| Endpoint | `GET https://app-dqs7nnc9k54x-api-AalZkPVdZX8L-gateway.appmiaoda.com/v1/images/editing/expand` |
| Content-Type | `application/json` |
...
生成期（Agent 直接调用）已改为脚本调用方式，不再使用本节 API 的内联调用代码。具体命令示例见 `SKILL.md` 的"生成期用法（Agent 直接调用）"一节（对应脚本 `scripts/generate_image_expand.py` 和 `scripts/query_image_expand.py`）。
...
async function pollTask(taskId: string, apiKey: string): Promise<TaskDetail> {
  ...
  while (Date.now() < deadline) {
    ...
    const res = await fetch(
      `https://app-dqs7nnc9k54x-api-rLobR6vwZJJ9-gateway.appmiaoda.com/v1/images/editing/expand/${encodeURIComponent(taskId)}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const createRes = await fetch(
      "https://app-dqs7nnc9k54x-api-Q9KWnzwVQMk9-gateway.appmiaoda.com/v1/images/editing/expand",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
7. **调用注意**：当前 `enable_billing: false`，实际计费以平台配置为准，避免不必要的重复提交。
