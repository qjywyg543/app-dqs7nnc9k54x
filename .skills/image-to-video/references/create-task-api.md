...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-m9xKX785MdZa-gateway.appmiaoda.com/beta/video/generations/kling/image2video` |
| Auth 模式 | `platform_managed`（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/json` |
| third_part_domain | `app-dqs7nnc9k54x-api-m9xKX785MdZa-gateway.appmiaoda.com` |
...
- **调用注意：** 本接口启用计费（`enable_billing: true`，`need_count_calls: true`），请避免不必要的重复调用。
