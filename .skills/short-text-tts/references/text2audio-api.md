...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-e94GZ5j0ljja-gateway.appmiaoda.com/text2audio` |
| 认证模式 | `platform_managed` |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/x-www-form-urlencoded` |
| Third-party Domain | `app-dqs7nnc9k54x-api-e94GZ5j0ljja-gateway.appmiaoda.com` |
...
```bash
python3 <skill-path>/scripts/call_text_to_audio.py --text "百度你好" --output /path/to/output.mp3
```

脚本内部负责读取 `INTEGRATIONS_API_KEY`、发送表单编码请求、根据响应 `Content-Type` 区分二进制音频与 JSON 错误，并将音频写入本地文件。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch("https://app-dqs7nnc9k54x-api-e94GZ5j0ljja-gateway.appmiaoda.com/text2audio", {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
...
Deno.serve(async (req) => {
  ...
  return fetch("https://app-dqs7nnc9k54x-api-e94GZ5j0ljja-gateway.appmiaoda.com/text2audio", {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
