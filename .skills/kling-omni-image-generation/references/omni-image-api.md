...
| Third-party Domain | `app-dqs7nnc9k54x-api-DLEO4zbkvoea-gateway.appmiaoda.com` |
...
| 方法 | POST |
| Endpoint | `https://app-dqs7nnc9k54x-api-DLEO4zbkvoea-gateway.appmiaoda.com/v1/images/omni-image` |
...
| 方法 | GET |
| Endpoint | `https://app-dqs7nnc9k54x-api-79jK6nw4zxDL-gateway.appmiaoda.com/v1/images/omni-image/{id}` |
...
生成期（Agent 直接调用）已改为脚本调用方式，不再使用本节 API 的内联调用代码。具体命令示例见 `SKILL.md` 的"生成期用法（Agent 直接调用）"一节（对应脚本 `scripts/generate_omni_image.py` 和 `scripts/query_omni_image.py`）。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-DLEO4zbkvoea-gateway.appmiaoda.com/v1/images/omni-image",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
serve(async (req: Request): Promise<Response> => {
  ...
  // --- 调用上游查询接口 ---
  const upstream = await fetch(
    `https://app-dqs7nnc9k54x-api-79jK6nw4zxDL-gateway.appmiaoda.com/v1/images/omni-image/${taskId}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
...
6. **计费**：创建任务接口按调用次数计费，轮询查询接口不计费，请避免不必要的重复提交。
...
| 方法 | POST |
| Endpoint | `https://app-dqs7nnc9k54x-api-eLMlPzV7qWJ9-gateway.appmiaoda.com/v1/images/omni-image` |
...
async function submitOmniImageEditTask(
  ...
  const response = await fetch(
    "https://app-dqs7nnc9k54x-api-eLMlPzV7qWJ9-gateway.appmiaoda.com/v1/images/omni-image",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
