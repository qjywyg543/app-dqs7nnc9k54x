...
生成期调用请使用内置脚本，脚本会从环境变量读取 INTEGRATIONS_API_KEY，不再现场处理图片编码和 SSE 协议。
...
也可以传入公网图片 URL 或完整消息数组：

```bash
python3 <skill-path>/scripts/call_multimodal_chat.py --text "这张图里有哪些文字？" --image-url "https://example.com/image.png"
```
