...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-k93RZBjPykEa-gateway.appmiaoda.com/v2/chat/completions` |
| 认证模式 | `platform_managed` |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/json` |
| 响应类型 | SSE 流式（`text/event-stream`） |
| third_part_domain | `app-dqs7nnc9k54x-api-k93RZBjPykEa-gateway.appmiaoda.com` |
...
```bash
python3 <skill-path>/scripts/call_multimodal_chat.py --text "请描述这张图片的主要内容" --image /path/to/image.jpg
```

脚本内部负责读取 `INTEGRATIONS_API_KEY`、编码本地图片、发送请求、解析 SSE 并聚合 `choices[].delta.content`。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-k93RZBjPykEa-gateway.appmiaoda.com/v2/chat/completions",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
- 前端只与 Supabase Edge Function 通信，不直接访问 `app-dqs7nnc9k54x-api-k93RZBjPykEa-gateway.appmiaoda.com`
