...
```bash
python3 scripts/call_ai_ppt_generator.py --query "生成一份新能源汽车市场分析 PPT"
```
...
```json
{"status":"succeed","result":{"ppt_url":"https://...","cover_urls":["https://..."]}}
```

完整 API 参数和流式响应字段说明见 `references/command-ppt-api.md`；生成期调用实现以 `scripts/call_ai_ppt_generator.py` 为准。
