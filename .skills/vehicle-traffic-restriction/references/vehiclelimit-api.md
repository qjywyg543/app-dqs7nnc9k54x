...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-pLVzAxRQyMWL-gateway.appmiaoda.com/vehiclelimit/query` | `POST https://app-dqs7nnc9k54x-api-DYJwnJVBwb4a-gateway.appmiaoda.com/vehiclelimit/city` |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` | 同左 |
| Content-Type | `application/json;charset=UTF-8` | `application/json;charset=UTF-8` |
| third_part_domain | `app-dqs7nnc9k54x-api-pLVzAxRQyMWL-gateway.appmiaoda.com` | 同左 |
| 计费 | 是 | 否 |
...
```bash
# 查询城市限行规则
python3 scripts/call_vehicle_limit.py query --city hangzhou --date 2026-04-27

# 获取支持限行查询的城市列表
python3 scripts/call_vehicle_limit.py cities
```

脚本会读取 `INTEGRATIONS_API_KEY`，并输出一行 JSON。
...
serve(async (req: Request): Promise<Response> => {
  ...
  // --- 调用上游 ---
  const url = new URL(
    "https://app-dqs7nnc9k54x-api-pLVzAxRQyMWL-gateway.appmiaoda.com/vehiclelimit/query"
  );
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-DYJwnJVBwb4a-gateway.appmiaoda.com/vehiclelimit/city",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json;charset=UTF-8",
...
- **调用注意：** 城市限行查询（`vehiclelimit/query`）按调用次数计费。
