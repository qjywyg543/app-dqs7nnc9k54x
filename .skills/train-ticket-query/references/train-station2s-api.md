...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-V9PworyO6gEa-gateway.appmiaoda.com/train/station2s` |
| 认证方式 | platform_managed（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer ${apiKey}` |
| Content-Type | `application/json;charset=UTF-8` |
| Third-party Domain | `app-dqs7nnc9k54x-api-V9PworyO6gEa-gateway.appmiaoda.com` |
...
```bash
python3 <skill-path>/scripts/query_train_station2s.py --start "杭州东" --end "北京南" --ishigh "1" --date "2024-01-01"
```

脚本内部负责读取 `INTEGRATIONS_API_KEY`、拼接 `start`/`end`/`ishigh`/`date` 查询参数、发送请求并输出 `result` 字段。这里不再维护内联请求代码，避免生成期重复消耗 token。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-dqs7nnc9k54x-api-V9PworyO6gEa-gateway.appmiaoda.com/train/station2s?${params}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json;charset=UTF-8",
