...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-Xa6JZxjyqrGa-gateway.appmiaoda.com/rest/2.0/ocr/v1/bus_ticket` |
| Auth 模式 | platform_managed |
| Auth Header | `X-Gateway-Authorization: Bearer <INTEGRATIONS_API_KEY>` |
| Content-Type | `application/x-www-form-urlencoded` |
| third_part_domain | `app-dqs7nnc9k54x-api-Xa6JZxjyqrGa-gateway.appmiaoda.com` |
...
# PDF / OFD 文件
python3 <skill-path>/scripts/call_bus_ticket_ocr.py --pdf-file /path/to/doc.pdf [--pdf-file-num 1]
python3 <skill-path>/scripts/call_bus_ticket_ocr.py --ofd-file /path/to/doc.ofd [--ofd-file-num 1]
```

脚本会按 `image > url > pdf_file > ofd_file` 优先级处理输入，成功时 stdout 输出一行 JSON（即上游接口原始响应，含 `words_result` 等字段）。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-Xa6JZxjyqrGa-gateway.appmiaoda.com/rest/2.0/ocr/v1/bus_ticket",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
