...
| third_part_domain | `app-dqs7nnc9k54x-api-DY8MN3QBydBa-gateway.appmiaoda.com` |
...
| 方法 | POST |
| Endpoint | `https://app-dqs7nnc9k54x-api-DY8MN3QBydBa-gateway.appmiaoda.com/v1/videos/image2video` |
| 计费 | 启用（按调用次数计费） |
...
| 方法 | GET |
| Endpoint | `https://app-dqs7nnc9k54x-api-zYkZzgKook1L-gateway.appmiaoda.com/v1/videos/image2video/{id}` |
| 计费 | 不启用 |
...
| 方法 | GET |
| Endpoint | `https://app-dqs7nnc9k54x-api-n9QVoDJ6oykL-gateway.appmiaoda.com/v1/videos/image2video` |
| 计费 | 不启用 |
...
生成期（Agent 直接调用）已改为脚本调用方式，不再使用本节 API 的内联调用代码。具体命令示例见 `SKILL.md` 的"生成期用法（Agent 直接调用）"一节（对应脚本 `scripts/generate_image_to_video.py` 和 `scripts/query_image_to_video.py`）。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-DY8MN3QBydBa-gateway.appmiaoda.com/v1/videos/image2video",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
serve(async (req: Request): Promise<Response> => {
  ...
  // --- 查询任务 ---
  const upstream = await fetch(
    `https://app-dqs7nnc9k54x-api-zYkZzgKook1L-gateway.appmiaoda.com/v1/videos/image2video/${task_id}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
...
### 计费
- **创建任务接口**（`api-DY8MN3QBydBa`）：按调用次数计费
- **查询接口**（单个 + 列表）：不计费
