...
直接调用 `scripts/query_company_info.py`，脚本从环境变量 `INTEGRATIONS_API_KEY` 读取密钥，并设置 `X-Gateway-Authorization: Bearer <key>`。

Bash 工具调用时 timeout 必须设置为 600000ms（600 秒）。

```bash
python3 scripts/query_company_info.py --keyword "北京百度网讯科技有限公司"
```

成功时 stdout 输出一行 JSON：

```json
{"code":200,"msg":"成功","success":true,"data":{"orderNo":"...","data":{"base":{"companyName":"北京百度网讯科技有限公司"}}}}
```
