...
| Endpoint | `GET https://app-dqs7nnc9k54x-api-e94GZ5j0Kxja-gateway.appmiaoda.com/business4/get` |
| Auth 模式 | platform_managed |
| Auth Header | `X-Gateway-Authorization: Bearer ${apiKey}` |
| Content-Type | `application/json;charset=UTF-8` |
| third_part_domain | `app-dqs7nnc9k54x-api-e94GZ5j0Kxja-gateway.appmiaoda.com` |
...
生成期调用请参考 `../SKILL.md` 和 `../scripts/query_company_info.py`，不要在 reference 中维护内联调用代码。API 详情、参数表、Edge Function 与前端示例保留在本文档。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstreamUrl = new URL(
    "https://app-dqs7nnc9k54x-api-e94GZ5j0Kxja-gateway.appmiaoda.com/business4/get"
  );
...
3. **计费**：每次查询均计费（`enable_billing: true`），避免重复查询相同公司；建议在前端缓存查询结果或加防抖处理。
