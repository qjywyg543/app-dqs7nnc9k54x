...
| Endpoint | `GET https://app-dqs7nnc9k54x-api-o9wN0pyVE2ea-gateway.appmiaoda.com/v1/videos/omni-video/{id}` |
| Auth 模式 | `platform_managed`（`traefik: true`） |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/json` |
| third_part_domain | `app-dqs7nnc9k54x-api-o9wN0pyVE2ea-gateway.appmiaoda.com` |
...
生成期（Agent 直接调用）已改为脚本调用方式，不再使用本节 API 的内联调用代码。具体命令示例见 `SKILL.md` 的"生成期用法（Agent 直接调用）"一节（对应脚本 `scripts/generate_omni_video.py` 和 `scripts/query_omni_video.py`）。
...
serve(async (req: Request): Promise<Response> => {
  ...
  let queryUrl: string;
  if (taskId) {
    queryUrl = `https://app-dqs7nnc9k54x-api-o9wN0pyVE2ea-gateway.appmiaoda.com/v1/videos/omni-video/${taskId}`;
  } else {
    queryUrl = `https://app-dqs7nnc9k54x-api-o9wN0pyVE2ea-gateway.appmiaoda.com/v1/videos/omni-video/${externalTaskId}?external_task_id=${externalTaskId}`;
  }
...
- **说明**：本接口用于轮询任务状态，轮询本身不计费，但避免不必要的重复任务提交。
