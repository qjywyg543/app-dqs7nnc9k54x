...
| Endpoint | `POST https://app-dqs7nnc9k54x-api-ra5EZDjVKkXa-gateway.appmiaoda.com/image-generation/submit` |
| Content-Type | `application/json` |
| third_part_domain | `app-dqs7nnc9k54x-api-ra5EZDjVKkXa-gateway.appmiaoda.com` |
| 计费 | 启用，按调用次数计费 |
...
| Endpoint（Web） | `POST https://app-dqs7nnc9k54x-api-VaOwP2jDmAga-gateway.appmiaoda.com/image-generation/task` |
| Endpoint（MiniProgram） | `GET https://app-dqs7nnc9k54x-api-VaOwP2jDmAga-gateway.appmiaoda.com/image-generation/task?taskId={taskId}` |
| Content-Type | `application/json` |
| third_part_domain | `app-dqs7nnc9k54x-api-VaOwP2jDmAga-gateway.appmiaoda.com` |
| 计费 | 不启用 |
...
生成期（Agent 直接调用）已改为脚本调用方式，不再使用本节内联调用代码。具体命令示例见 `SKILL.md` 的"生成期用法（Agent 直接调用）"一节（对应脚本 `scripts/generate_image_advanced.py` 和 `scripts/query_image_advanced.py`）。
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-ra5EZDjVKkXa-gateway.appmiaoda.com/image-generation/submit",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-dqs7nnc9k54x-api-VaOwP2jDmAga-gateway.appmiaoda.com/image-generation/task",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
2. **计费**：提交接口（`api-ra5EZDjVKkXa`）启用计费；
